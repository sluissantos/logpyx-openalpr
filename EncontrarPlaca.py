...
def desenhaContornos(contornos, imagem):
    for c in contornos:
        ...
        if (perimetro > 240) and (perimetro < 500):
            ...
            if len(approx) == 4:
                ...
                cv2.rectangle(imagem, (x, y), (x + lar, y + alt), (0, 255, 0), )
...
def buscaRetanguloPlaca(source):
    ...
    while video.isOpened():
        ...
        if frame_count == 10:
            ...
            cv2.imshow("Escala Cinza", img_result)

            # limiarização
            ret, img_result = cv2.threshold(img_result, 70, 255, cv2.THRESH_BINARY)
            cv2.imshow("Limiar", img_result)
            # desfoque
...
def preProcessamentoRoi(img_roi):
    ...
    img = cv2.resize(img_roi, None, fx=5, fy=5, interpolation=cv2.INTER_CUBIC)
...
def reconhecimentoOCR(img_roi_ocr):
